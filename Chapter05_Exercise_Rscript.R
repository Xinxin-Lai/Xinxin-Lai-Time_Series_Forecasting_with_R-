...
#There is a posiTive relationshup btw the two variables. Higher temperature, more demand of electricity.
...
#From the plot this model is adequate because residuals aren't correlated with each other. But there was an outlier.
...
fc_Dem_Temp
# The model predicts correctly because the forecast temperature values were close to the range the actual ones in the data
...
elecdaily %>%
  ...
  geom_point() +
  geom_smooth(method="lm", se=FALSE)
# The plot shows that the model was built by few data points. It could explain well of the first 20 days, but not a good model for the total data points.
...
autoplot(mens400)
# Feature1:Winning times in Plympic men's400m track final had the trend of dereasing as time went on.
...
# First extract time part from mens400 time series to do linear modeling.
...
# The winning times have been decreasing at average rate of 0.0457 second /year.
...
# The residuals plot shows that the model generally fit the data well as they spread around 0. We can check by checkresiduals function, too:
checkresiduals(tslm_mens400)

# d.Predict the winning time for the men’s 400 meters final in the 2020 Olympics. Give a prediction interval for your
...
fc_mens400$upper
fc_mens400$lower
# 80% interval is from 43.63 to 44.53
# 95% interval is from 39.55 to 40.45
# But we need to consider that they were calculated from the assumption that the model's
# residuals were normally distributed. But we saw from the result of checkresiduals function that it isn't true.

#3.Type easter(ausbeer) and interpret what you see.
help("ausbeer")
head(ausbeer)
easter(ausbeer)
# easter function returns a vector of 0's or 1's or fractional parts in the observed 
# time period. If full Easter holidays are in a time period, it returns 1, and returns
# 0 otherwise. If the holidays are extended from a period to the other, easter function returns fractional portions to each of them.

# 5.The data set fancy concerns the monthly sales figures of a shop which opened in January 1987 and sells gifts,
# souvenirs, and novelties. The shop is situated on the wharf at a beach resort town in Queensland, Australia. 
# The sales volume varies with the seasonal population of tourists. There is a large influx of visitors to the
# town at Christmas and for the local surfing festival, held every March since 1988. Over time, the shop has expanded its premises, range of products, and staff
...
head(fancy)
autoplot(fancy)
# There is upward trend with a spike every year during Christmas season, and a slight decrease in 1991.
# b.Explain why it is necessary to take logarithms of these data before fitting a model.
# Because there isn't constant mean and variance in fancy data, it's increased exponentially. Therefore it's necessary to take logarithms of the data.
...
# If I had made surfing_festival as a list, I should've needed to use unlist function to make it as atomic vector, 
# not nested list. tslm function can get vector or factor type data, but it cannot get nested list.
...
# The residuals have pattern against time. It means that there is correlation btw residuals and time.
...
cbind(Residuals = tslm_log_fancy$residuals,
  ...
  ggplot(aes(x = Fitted_values,
             y = Residuals)) +
  geom_point()
# The size of the residuals changed as we move along the x-axis(fitted values). It means that even after the log transformation, there are still heteroscedacity in the errors or that the variance of the residuals aren't still constant.
...
cbind.data.frame(
  ...
  ggplot(aes(x = Month,
             y = Residuals)) +
  geom_boxplot()
# If vectors are combined by cbind function, the class of the result is matrix, which 
# should hold one type of data. If I want to make the columns to have different data types, 
# I need to use cbind.data.frame function instead. Instead, if I still want to use cbind function,
# I need to use as.numeric function in mapping of ggplot.
# If the mapping of boxplot is (factor x factor), it would be difficult to see any box because boxplot function 
# can't aggregate factor type data. The result would be looked like a scatter plot.
...
# The distribution of the residuals was unsymetrical for some months. And for some months, the 
# median of the residuals wasn't 0(residuals' mean should be 0 for all months because getting
# the minimum SSE means getting mean). Residuals with such properties can't have normal distribution, 
# which will make it difficult to get prediction interval and can be double-check by checkresiduals function too.

# f. The distribution of the residuals was unsymetrical for some months. And for some months, the median of the residuals wasn't 0(residuals' mean should be 0 for all months because getting the minimum SSE means getting mean). Residuals with such properties can't have normal distribution, which will make it difficult to get prediction interval.
...
# And all seasonal variables are positive. It means that the sales amount was minimum on January as the base and the sales of the other months were greater than January for most of years. 
...
checkresiduals(tslm_log_fancy)
# The p-value is <0.05, which means that the residuals can be distinguished from te white noise, and the resiudals can be correlated with each other.

# h.Regardless of your answers to the above questions, use your regression model to predict the monthly
# sales for 1994, 1995, and 1996. Produce prediction intervals for each of your forecasts
...
# The itnervals in Decembers were larger than the other months'.
...
#extract gasoline data up to end of 2004 and plot it:
...
for(num in c(1,2,3,5,10,20)){
  ...
  print(
    autoplot(gasoline_until_2004) + 
      ...
      ggtitle(var_name) +
      ylab("gasoline") +
      guides(color = guide_legend(title = "Number of Fouruer Transform pairs"))
  )
}
...
for (i in c(1,2,3,5,10,20)){
  tslm_ft1_gasoline_until_2004.name <- paste(
    "tslm_ft",as.character(i), "_gasoline_until_2004",
    sep = ""
  )
  writeLines(
    paste(
      "\n",tslm_ft1_gasoline_until_2004.name,"\n"
    )
  )
  print(CV(get(tslm_ft1_gasoline_until_2004.name)))
}
...
#c.Check the residuals of the final model using the checkresiduals() function. Even though the residuals fail the correlation tests, the results are probably not severe enough to make much difference to the forecasts and prediction intervals.
...
str(huron)
head(huron)
# There are lots of fluctuations that show some cyclic behavior. And up to about 1930,there were downward trend, but after that the trend disappeared.
...
# It looked like spline model didn't catch the trend well. Linear model and log transformed linear model couldn't 
# reflect the trend change around the year 1915.
...
#Q4 & Q8 are for advanced math reader not related to coding and interpretation,thus I skipped here.
# References:
# 1. https://otexts.com/fpp2/regression-exercises.html
# 2. https://github.com/JehyeonHeo/Forecasting_with_R_practices/blob/master/Chapter5.rmd
